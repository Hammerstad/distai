...
public class TournamentBot4 extends Agent {
	...
	private boolean createAgents() {
		...
		try {
			...
			AgentController g = d.createNewAgent("MixtureAgent", "MixtureAgent", null);
			a.start();
			b.start();
			c.start();
			e.start();
	...
	private void runTournamentBetweenTwoAgents(int rounds, String one, String two) {
		presentPrisonersWithDilemma(one, two);
		receiveReply(rounds, one, two);
	}

	private void presentPrisonersWithDilemma(final String one, final String two) {
		System.out.println("\n");
	...
	private void receiveReply(final int rounds, final String one, final String two) {
		final String[] answer = new String[2];
		final int[] score = new int[2];

		while (state == State.WAITING_FOR_REPLY) {

			// System.out.println("Trying to receive.");
			MessageTemplate m1 = MessageTemplate.MatchPerformative(ACLMessage.PROPOSE);
			ACLMessage message = null;
			try {
				message = blockingReceive(m1, 10);
			} catch (Exception e) {
			...
			if (answer[0] != null && answer[1] != null) {
				...
				score[0] = temp[0];
				score[1] = temp[1];
				System.err.println(rounds + " :: " + new AID(one, AID.ISLOCALNAME).getName() + " and " + new AID(two, AID.ISLOCALNAME).getName()
						+ ": " + Arrays.toString(temp));
				state = State.INFORM_ABOUT_SCORE;
			}
		}
		informAboutScore(one, two, score);
		state = State.ADD_SCORE;
		addScore(rounds, one, two, score);

		System.out.println("Checking if we're done: " + (score[0] != 0 || score[1] != 0));

	}

	private void informAboutScore(final String one, final String two, final int[] score) {
	...
	private void addScore(final int rounds, final String one, final String two, final int[] score) {
		battles++;
